...
def main(pais):   
    ...
    df_time_inference.to_sql(name='time_inference_dbn_comum_'+str(pais), con = get_connection(),schema = 'results', if_exists = 'replace', chunksize = None, index = False)
    forecast_values.to_sql(name='forecast_dbn_comum_'+str(pais), con = get_connection(),schema = 'results', if_exists = 'replace', chunksize = None, index = False)
