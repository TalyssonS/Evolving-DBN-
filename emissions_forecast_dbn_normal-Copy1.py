...
def main(pais):   
    ...
    #dataset to learn the model using the first 7 days
    i = dates[0]
    data_learn = get_dataset(pais,i, i+timedelta(days = 6))
    ...
    df_time_inference.to_sql(name='time_inference_dbn_comum_2'+str(pais), con = get_connection(),schema = 'results', if_exists = 'replace', chunksize = None, index = False)
    forecast_values.to_sql(name='forecast_dbn_comum_2'+str(pais), con = get_connection(),schema = 'results', if_exists = 'replace', chunksize = None, index = False)
